...
class UsersControllerTest {
    ...
    void users() throws Exception {
        when(service.getUsers()).thenReturn(List.of(User.builder().build()));
    ...
    @Test
    void getUserByAccessAndAlbumId() throws Exception {
        when(service.getUserByAccessAndAlbumId(anyString(), anyString())).thenReturn(List.of(User.builder().build()));

        mockMvc.perform(MockMvcRequestBuilders
                        .get("/api/v1/users", AccessEnum.READ, "1")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
}